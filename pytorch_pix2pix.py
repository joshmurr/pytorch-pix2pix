...
parser.add_argument('--train_subfolder',
                    required=False,
                    default='train',
                    help='')
...
for epoch in range(opt.train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        D_real_loss = BCE_loss(D_result,
                               Variable(torch.ones(D_result.size()).cuda()))

        G_result = G(x_)
        D_result = D(x_, G_result).squeeze()
        D_fake_loss = BCE_loss(D_result,
                               Variable(torch.zeros(D_result.size()).cuda()))
        ...
        G_train_loss = BCE_loss(D_result,
                                Variable(torch.ones(D_result.size()).cuda())
                                ) + opt.L1_lambda * L1_loss(G_result, y_)
...
util.show_train_hist(train_hist,
                     save=True,
